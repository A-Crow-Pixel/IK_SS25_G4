"""
Chat application client module (version 1.1)
...
This is version 1.1 of the client, with basically the same functionality as the main version.
...
"""
# Import PySide6 related modules for GUI
from PySide6.QtWidgets import *
from PySide6.QtCore import Signal, QObject, Qt
from PySide6.QtGui import QTextCursor, QTextBlockFormat
# Third-party packages for threading, networking, translation, language detection, etc.
...
import time
# Import reminder popup components
from rrd_widgets import TipsWidget, TipsStatus
from client.gui.modern_main_window import ModernMainWindow
from client.gui.modern_reminder import ModernReminderDialog
from client.gui.modern_dialogs import (
    ModernConnectToServerDialog,
    ModernAddDialog,
    ModernModifyGroupDialog,
    ModernInvitePopUpDialog
)

# Signal class for communication between UI thread and worker thread
class MySignals(QObject):
    # Different UI component and string content signals
...
class Stats:
    ...
    def handleTestTransButton(self):
        ...
        if self.Socketm.connected and self.Socketm.tcp_socket:
            ...
            translate_msg.original_text = "This is a message to test the translation functionality"
            # Don't set translated_text field
    ...
    def on_chat_target_changed(self):
        ...
        item = self.ui.UserGroupTree.currentItem()
        if item:
            # Use data to get user ID or group ID, not text to get display text (may contain unread message marker)
            item_data = item.data(0, Qt.UserRole)
            if item_data and len(item_data) >= 2:
                chat_id = item_data[1]  # [0] type, [1] user_id or group_id
                # Use chat_id directly as display name to avoid unread message marker
                self.signal.nameofchatLabel.emit(self.ui.NameOfChat, chat_id)

                # Switch to the corresponding chat window
    ...
    def handleSetReminder(self):
        ...
        # Clear input box and close dialog
    ...
    def show_reminder_popup(self, message):
        ...
        """
        # print("show_reminder_popup函数")  # Commented out to reduce console output
    ...
    def show_group_invite_popup(self, group_id, server_id, handle):
        ...
        # Disconnect previous connection to avoid duplicate binding
        try:
            self.dialog4.YesButton.clicked.disconnect()
            self.dialog4.NoButton.clicked.disconnect()
        except:
            pass  # If no connection, ignore
...
class Socketmanager:
    def __init__(self, dialog_ref, server_list, tcp_ref, dialog_ref1, ui_ref, dialog_ref2, dialog_ref3):
        ...
        self.last_active_time = 0  # Last active time
        self.heartbeat_interval = 30  # Heartbeat interval
        self.heartbeat_timeout = 90   # Timeout
        self.search_users_unit64id = None

        # Various signals and UI widget bindings
        ...
        self.current_chat_id = None  # Current selected chat object ID
    ...
    def update_tree_display(self, chat_id):
        ...
        # First, find all top-level items (private chats or groups)
        ...
        # If not found in top-level items, search for members in groups
        for i in range(root.childCount()):
            item = root.child(i)
            # Check group members
    ...
    # Update text display for a single item
    def _update_item_text(self, item, chat_id):
        ...
        if unread_count > 0:
            # Limit the maximum number displayed, show 99+ if more than 99
            display_count = "99+" if unread_count > 99 else str(unread_count)
            
            # Use a unified style of red circle + number
    ...
    def add_message_to_chat(self, chat_id, sender, message, is_me=False):
        ...
        # Unread message handling: if it's not a message I sent and not the current chat window, increment unread count
    ...
    # Chat window update display (supports multiple windows)
    def update_ChatMainWindow(self, sender, message, is_me=False):
        # Get current chat object ID
        if self.current_chat_id:
            # Safely add message to chat window using signal
    ...
    # UDP broadcast to discover server
    def UDPBoard(self):
        self.dialog1.ServerTable.setRowCount(0)
        ...
        for broadcast_addr in broadcast_addrs:
            try:
                ...
                # print(f"[Client] sendto {broadcast_addr}:{udp_port} a DISCOVER_SERVER")  # Commented out to reduce console output
                ...
                try:
                    ...
                    if purpose == "SERVER_ANNOUNCE":
                        ...
                        servers_found = True
                        # print(f"[Client] Received server response from {server_addr}")  # Commented out to reduce console output
                        break  # Stop when server is found
                except TimeoutError:
                    continue  # Continue to next broadcast address
            except Exception as e:
                # print(f"[Client] Broadcasting to {broadcast_addr} failed: {e}")  # Commented out to reduce console output
                continue
    ...
    def close_connection(self):
        if self.tcp_socket:
            try:
                ...
                # print("[Client] Socket close error:", e)  # Commented out to reduce console output
                pass
            self.tcp_socket = None
        self.connected = False


    # Establish TCP connection, responsible for sending connection request, starting receive/heartbeat threads
    def TCPConnect(self):
        if self.connected:
        ...
        try:
            # print("[Client] Opening new TCP socket")  # Commented out to reduce console output
    ...
    # Heartbeat thread, periodically checks connection status and activity
    def heartbeat_loop(self):
        while self.connected and self.tcp_socket:
            ...
            try:
                ...
                # print("[Client] Sent PING")  # Commented out to reduce console output
            except Exception as e:
                # print("[Client] Heartbeat send error:", e)  # Commented out to reduce console output
                self.close_connection()
                break

    # TCP message main receive thread, processes all received protocol packets
    def recv_loop(self):
        while self.connected and self.tcp_socket:
            ...
            # print("[Client] Purpose:", purpose)  # Commented out to reduce console output
            ...
            if purpose == 'CONNECTED':
                ...
                # print("[Client] Received PING, sent PONG")  # Commented out to reduce console output
            elif purpose == 'PONG':
                # print("[Client] Received PONG (heartbeat ok)")  # Commented out to reduce console output
                pass
                ...
                if self.search_users_unit64id != QueryUserResponse.handle:
                    # print(self.search_users_unit64id)  # Commented out to reduce console output
                    pass
                else:
                    ...
                    if input_userid in [user.userId for user in QueryUserResponse.users]:
                        # print('有')  # Commented out to reduce console output
                ...
                if recipient_type == 'user':
                    # First, add sender to user tree (if not already present)
                    self.signals.add_tree_user.emit(sender, sender_server)
                    
                    # Safely add message to sender's chat window using signal
                    self.signals.add_message_to_chat_signal.emit(sender, sender, msg_text, is_me)
                    
                    # If the currently selected chat is the sender, no extra action is needed (as the window is already switched)
                    # The message will automatically appear in the current window
                    ...
                    # Safely add message to group's chat window using signal
                ...
                for status in ack.statuses:
                    # print("收信人", status.user.userId, "投递状态：", status.status)  # Commented out to reduce console output
                    pass
                ...
                if resp.result == Message_pb2.ModifyGroupResponse.SUCCESS:
                    ...
                    # Safely close dialog using signal
                ...
                # Display group invite popup in the main thread using signal
                ...
                if existing_group_item:
                    ...
                    # Clear existing members (but keep the group itself)
                    ...
                    # Check if self is in the member list, if not, add
                ...
                try:
                    ...
                    # print('test 111')  # Commented out to reduce console output
                    global_signal.show_reminder_popup.emit(message)
                    
                except Exception as e:
                    # print(f"[Client] REMINDER error: {e}")  # Commented out to reduce console output
                    pass

            else:
                # Process other unknown messages
                # print(f"[Client] Received unhandled message: {purpose}")  # Commented out to reduce console output
                pass
    ...
    # Search for users, request user list from server
    ...
    # Send message to currently selected user/group
    def send_message(self):
        ...
        original_text = self.ui.InputTextEdit.editer.text()
        ...
        if selected_language == "Original":
            ...
            # Local display of translated content
    ...
    # Initiate group creation request to server
        ...
        if group_id in self.chat_browsers:
            ...
            if self.current_chat_id == group_id:
                ...
                if len(self.chat_browsers) > 1:
                    # Find the first chat window that is not the current group's chat window
                    for chat_id in self.chat_browsers:
                        if chat_id != group_id:
                            self.switch_chat_window(chat_id)
                            break
                else:
                    # No other chat windows, switch to the default blank window
    ...
    # Send set reminder message
    def send_set_reminder(self, event_name, countdown_seconds):
        if not self.tcp_socket:
            return
        
        set_reminder = Message_pb2.SetReminder()
        # Users can only set reminders for themselves, so the target user is the current user
