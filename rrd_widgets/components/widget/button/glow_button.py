...
class GlowButton(ButtonAnimationBase):
    ...
    def setBorder(self):
        btn_x = self.border_radius / 2
        btn_y = self.border_radius / 2
        btn_width = self.width() - self.border_radius
        btn_height = self.height() - self.border_radius
        return QRect(btn_x, btn_y, btn_width, btn_height)

    def paintEvent(self, event):
        super().paintEvent(event)
        path = QPainterPath()
        path.addRoundedRect(self.setBorder(), self.border_radius / 2, self.border_radius / 2)
