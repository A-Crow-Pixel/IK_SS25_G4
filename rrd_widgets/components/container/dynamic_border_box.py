...
from ..base import WidgetAnimationBase
...
class DynamicBorderWidget(WidgetAnimationBase):
    ...
    def drawForeground(self, painter: QPainter):
        painter.save()
        gradient = QConicalGradient(self.width() / 2, self.height() / 2, 0)  # Set conical gradient origin to card center
        gradient.setColorAt(0, QColor(0, 0, 0, 255))
        gradient.setColorAt(0.5, self.color_1)
        painter.setBrush(gradient)
        painter.rotate(self.angle)  # Rotate brush
