...
from rrd_widgets.components.layout.grid_layout import RGridLayout
...
class RExpandButtonBox(QFrame):
    def __init__(self, parent):
        ...
        self.btn_background_color: QColor = QColor()
        self.paramsInit()
        self.widgetInit()
    ...
    def setParams(self, btn_background_color):
        self.btn_background_color = btn_background_color

    def paramsInit(self):
        self.locatorBoxSize: QSize = QSize()  # Locator frame size
        self.locatorBoxWidgetWidth: int = 0  # Locator button width
        self.locatorBoxWidgetSpacing: int = 0  # Spacing between locator buttons

        self.expandBoxWidgetSize: QSize = QSize()  # Expand component size
        self.expandBoxWidgetWidth: int = 0  # Button width inside expand component
        self.expandBoxWidgetSpacing: int = 0  # Button spacing inside expand component

        self.animationDuration: int = 0  # Animation duration for each button
        self.timeOffset = 50  # Animation interval
    ...
    def setConfig(self):
        ...
        self.ui()  # Cannot be called in paintEvent, showEvent
    ...
    def animationParams(self):
        ...
        :return:
        '''
        # Forward animation radius end value
        self.radius_end = self.locatorBoxWidgetWidth // 2
        # Forward animation color end value
        self.color_end = "rgba(255, 255, 255,255)"
        # Forward animation end button size value
        self.size_end = QSize(self.locatorBoxWidgetWidth, self.locatorBoxWidgetWidth)

        # Reverse animation radius end value
        self.radius_end_reverse = self.expandBoxWidgetWidth // 2
        # Reverse animation color end value
        self.color_end_reverse = self.btn_background_color
        # Reverse animation end button size value
        self.size_end_reverse = QSize(self.expandBoxWidgetWidth, self.expandBoxWidgetWidth)

        # Determine expand/collapse
        self.is_fold = 0
        ...
        self.animationInit()
        self.animationList = [self.animationE, self.animationW, self.animationN, self.animationS, self.animationCenter,
                              self.animationNW, self.animationSE, self.animationNE, self.animationSW,
                              self.animationFrame]
    ...
    def animationForward_finished(self, pushButton):
        if isinstance(pushButton, RButton):
            pushButton.setStyleSheet(
                f"background-color: {self.color_end};"
                f"border-radius: {self.radius_end}px;"
            )
    ...
    def animationBackward_finished(self, pushButton):
        if isinstance(pushButton, RButton):
            pushButton.setStyleSheet(
                f"background-color: {self.color_end_reverse.name()};"
                f"border-radius: {self.radius_end_reverse}px;"
            )

            pushButton.pushButton.setGeometry(QRect(QPoint(0, 0), self.size_end_reverse))
    ...
    def getEndValue(self, pos):
        ...
        return QRect(self.frame_3.pos() + pos + QPoint(offset_x, offset_y), self.size_end)
